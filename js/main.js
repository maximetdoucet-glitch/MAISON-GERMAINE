(() => {
  ...
  if (bookModal) {
    ...
    // Salon-typical hours used to drive the demo time grid. Replace with the
    // shop's real hours before launch (they weren't public when this was built).
    const SHOP_HOURS = {
      2: [9, 18],   // Tue
      3: [9, 18],   // Wed
      4: [9, 18],   // Thu
    ...
    function buildIcsDataUri(start, end) {
      ...
      const uid = 'mg-' + Math.random().toString(36).slice(2) + '@maisongermaine.nl';
      const summary = `Maison Germaine — ${state.serviceLabel}`;
      const description = `Afspraak: ${state.serviceLabel}\\nStylist: ${state.barberLabel}\\nNaam: ${state.name}\\nPrijs: €${state.price}\\n\\nWillemsweg 13, 6531 DA Nijmegen`;

      const ics = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Maison Germaine//Booking//NL',
        'CALSCALE:GREGORIAN',
        'METHOD:PUBLISH',
        'BEGIN:VEVENT',
        `UID:${uid}`,
        ...
        'LOCATION:Willemsweg 13\\, 6531 DA Nijmegen',
        'END:VEVENT',
        'END:VCALENDAR'
      ].join('\r\n');
